...
public class RestAssuredExercises1Test {
	...
	public void requestUsZipCode90210_checkContentType_expectApplicationJson() {
		...
		given().
			spec(requestSpec).
			when().get("/us/90210").
			then().assertThat().contentType(equalTo("application/json"));
	}
	 ...
	 * use the following GPath expression:
	 * places[0].state
	 **********************************************/
	...
	public void requestDeZipCode24848_checkListOfPlaceNames_expectDoesNotContainFrankfurt() {
		...
		given().
			spec(requestSpec).
			when().get("/de/24848").
			then().assertThat().body("places.'place name'",not(hasItem("Frankfurt")));
	}
